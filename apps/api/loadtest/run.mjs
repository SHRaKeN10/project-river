...
// --- metrics ---
let msgsIn = 0, msgsOut = 0, handsDone = 0, actionsSent = 0, errors = 0;
...
async function main() {
  ...
  const admin = (await http('/api/auth/login', { method: 'POST', body: { emailOrUsername: 'loadadm', password: PW } })).json;
  let tables = [];
  const mk = await http('/api/tables', { method: 'POST', token: admin.tokens.accessToken, body: { name: `${sfx} t0`, smallBlind: 5, bigBlind: 10, maxSeats: PPT + 1, minBuyIn: 200, maxBuyIn: 2000 } });
  if (mk.status === 201) {
    tables.push(mk.json.id);
    for (let i = 1; i < NT; i++) {
      const r = await http('/api/tables', { method: 'POST', token: admin.tokens.accessToken, body: { name: `${sfx} t${i}`, smallBlind: 5, bigBlind: 10, maxSeats: PPT + 1, minBuyIn: 200, maxBuyIn: 2000 } });
  ...
  if (SEEDED.length < total) throw new Error(`need ${total} seeded users, have ${SEEDED.length}. run _seedusers.mjs`);
  ...
  for (const tableId of tables) {
    for (let seat = 0; seat < PPT; seat++) {
      ...
      s.on('table:state', (st) => {
        ...
        if (kinds.includes('CHECK')) act = roll < 0.9 ? { type: 'CHECK' } : { type: kinds.includes('BET') ? 'BET' : 'CHECK', amount: st.bigBlind * 2 };
        else if (kinds.includes('CALL')) act = roll < 0.75 ? { type: 'CALL' } : roll < 0.92 ? { type: 'FOLD' } : { type: kinds.includes('RAISE') ? 'RAISE' : 'CALL', amount: (st.currentBet || st.bigBlind) * 2 + st.bigBlind };
        else act = { type: 'FOLD' };
        const t0 = performance.now();
        actionsSent++; msgsOut++;
        s.emit('player:action', { tableId, handId: st.handId, clientSeq: ++seq, action: act }, (r) => {
          latencies.push(performance.now() - t0);
          if (r && r.error) errors++;
        });
      });
      s.on('hand:end', () => { handsDone++; });
      s.on('error', () => { errors++; });
  ...
  const pct = (q) => latencies.length ? latencies[Math.min(latencies.length - 1, Math.floor(latencies.length * q))].toFixed(1) : 'n/a';
  console.log('\n=== RESULTS ===');
  ...
  console.log(`action->ack  p50:   ${pct(0.5)}ms   p95: ${pct(0.95)}ms   p99: ${pct(0.99)}ms   max: ${latencies.at(-1)?.toFixed(1) ?? 'n/a'}ms`);
